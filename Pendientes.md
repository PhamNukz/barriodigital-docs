...
- [x] **G6** `P1` `Francisco + Benjamín` `.github/workflows/deploy.yml` en `barriodigital-infra`: on push `main` + `workflow_dispatch` → SSH a `ec2-apps` (`appleboy/ssh-action`) → escribe `.env` desde secret → `git pull` → `docker compose pull && docker compose up -d`. — infra
- [x] **G7** `P1` `Francisco + Benjamín` Secrets en `barriodigital-infra` (Settings → Secrets → Actions): `EC2_HOST` (EIP), `EC2_USER` (`ubuntu`), `EC2_SSH_KEY` (clave privada **dedicada al deploy**, no la `.pem` del key pair), `APPS_ENV` (contenido completo del `.env`). — GitHub
- [x] **G8** `P1` `Francisco + Benjamín` Generar la clave dedicada: `ssh-keygen -t ed25519 -f deploy_key -C barriodigital-deploy`, agregar la pública a `~/.ssh/authorized_keys` de `ec2-apps`, la privada al secret. — AWS, GitHub
